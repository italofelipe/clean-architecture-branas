import ClassroomRepositoryMemory from "./ClassRepositoryMemory";
...
test("Should generate enrollment code", function () {
	...
	expect(enrollment.code.value).toBe("2023EM1A0001");
});
...
test("Should generate invoices", function () {
	...
	expect(enrollment.invoices[enrollment.invoices.length - 1].amount).toBe(
		1416.73
	);
});
